! This is an update of the code at: https://github.com/mostafa-razavi/ITIC
! The code has been updated to read the second virial coefficient (svc) by On the Fly method (ofsvc)
! Refs.
! ITIC - Razavi, S. Mostafa, Richard A. Messerly, and J. Richard Elliott. "Coexistence calculation using the isothermal-isochoric integration method." Fluid Phase Equilibria 501 (2019): 112236. https://doi.org/10.1016/j.fluid.2019.06.026
! OFSVC - J. Richard Elliott. J. Phys. Chem. B, 125: 4494-4500 (2021).
! IticOfsvc1a: apply standard 9 points on isotherm, but ICs can be variable. 
!              ICs must be the highest density IT points, but you can have 3 or 4. 
!              Typically, choosing the highest density as saturated liquid at Tr~0.5 gives Tr~{0.5,0.6,0.7,0.8} 
program	main
use MSFLIB   !JRE20210620 for windows. Comment out for Linux.
use VirCoModule ! Contains B2calc, ReadB2().
implicit double precision(A-H,O-Z)
parameter(Rgas=8.3144598d0, zeroTol=1.d-11)

integer(2) iArg !JRE20210620 for windows. Comment out for Linux 
...
double precision :: Ures_IT(20),Mw
...
character*50 String,dumString1,dumString2 !,dumString3

maxTolerance = 1.0d0 

iArg = NARGS() ! iArg=1 if called from windows
if(iArg > 1)then  ! we should have two arguments from command line: (1) zures_datafile (2) ITIC_conditions
	iArg=1	!JRE20210620 necessary to split for windows
	call getarg(iArg,arg1)
	iArg=2	!JRE20210620 necessary to split for windows
	call getarg(iArg,arg2)
	zures_datafile = arg1
	ITIC_conditions = arg2
else					!execute in the dir where RegSteps is called from.
	zures_datafile='C8_MiPPE.zures'
...
nItPts = 9
nICs = 5
!read(2231,*,ioStat=ioErr)nItPts,nICs !JRE20210621 add this capability later.
do loopline=1,200	
	read(2231,*,ioStat=ioErr) dumString1,dumString2
	if(loopline < 15)print*,TRIM(dumString1),' ',TRIM(dumString2)
	String = trim(dumString1)
	if(String.eq."RHO_HIGH:") then
		read(dumString2,*,iostat=ioErr)highestRho
	elseif(trim(String).eq."MW:") then
		read(dumString2,*,iostat=ioErr) MW	
	elseif(trim(String).eq."TC:") then
		read(dumString2,*,iostat=ioErr) TC
	!elseif(String.eq."T_IT:") then	!JRE20210620. Ofsvc eliminates need for subcritical simulations.
	!	backspace 2231
	!	read(2231,*,ioStat=ioErr) dumString1,dumString2, dumString3
	!	read(dumString3,*,iostat=ioErr) T_IT_subcritical
	elseif(trim(String).eq."T_HIGH:") then
		read(dumString2,*,iostat=ioErr)HighestT	 !this is the isotherm.
	elseif(trim(String).eq."T_IC1:") then
		write(*,*)
		iFirstIcPt=nItPts-nICs+1 ! !JRE20210620 IticOfsvc1a: apply standard 9 points on isotherm, but nICs can be variable (2,3, or 4). 
	...
	if(ioErr /= 0 .and. loopline < 15)print*,'ITICmain: ioErr=',ioErr,' reading ',TRIM(dumString1),TRIM(dumString2)	!JRE20210620 
end do
close(2231)	
T_IT_subcritical=0.8d0*Tc !JRE20210620 We shouldn't need this but set it anyway for consistency with original version.

if(Tc < zeroTol)print*,'ITICmain: 0=Tc?=',Tc
SubCritReducedTemp = T_IT_subcritical / TC
if(Mw < zeroTol)print*,'ITICmain: 0=Mw?=',Mw
readCheck=Mw*Tc*highestRho*HighestT
if(readCheck < zeroTol)pause 'ITICmain: check values of highestRho,Mw,Tc,highestT'
...
!JRE20210620 eliminated by ofsvc
...
enddo
close(4321)
calculateVirialSupercritical=.FALSE. !JRE20210620
calculateVirialSubcritical=.FALSE.   !JRE20210620
RHO_IT_CALC_VR=0					 !JRE20210620 
RHO_IT_VR=0							 !JRE20210620 
RHO_IT2_VR=0						 !JRE20210620 
Z_IT_VR=0							 !JRE20210620 
Z_IT2_VR=0							 !JRE20210620 
T_IT_VR=0							 !JRE20210620 
T_IT2_VR=0							 !JRE20210620 
Ures_IT_VR=0						 !JRE20210620 
Ures_IT_VR2=0						 !JRE20210620 
B3Sat=0								 !JRE20210620 

if(.NOT. calculateVirialSupercritical)CALL ReadB2Coeffs(iErr) !JRE20210620 values passed in B2module. With B2coeffs, we can calculate B2 at any T. 
...
if(calculateVirialSupercritical) then  !JRE20210620 not happening
	write(*,*)
...
if(calculateVirialSubcritical) then	 !JRE20210620 not happening
	write(*,*)
...
B2atHighestT=B2calc(HighestT,TdB2_dTit)/Mw	!JRE20210620
B3atHighestT=0 							!JRE20210620, this constrains us to Tr < 0.8 for coexistence.
B2atSubcriticalIT=B2calc(T_IT_subcritical,TdB2_dTsub)/Mw	!JRE20210620. We don't really need this but calculate it anyway.
B3atSubcriticalIT=0 							!JRE20210620, this constrains us to Tr < 0.8 for coexistence.
TofSubCriticalIT=T_IT_subcritical       ! JRE20210622 Dunno why smr needs redundant def, but go with it.

if(calculateVirialSupercritical) then  !JRE20210620 we skip all this.
...
if(calculateVirialSupercritical .AND. calculateVirialSubcritical) then	!JRE20210620 not happening
...
!JRE20210620 integrate isotherm !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
write(*,*)
write(*,*)'===================== Integrating Isotherm ============================'
write(*,*)
...
write(*,'(a,11f7.4)')' rho(g/cc)=',(rho_IT(i),i=1,nItPts)
write(*,'(a,11f7.2)')' aDep_IT = ',(aDep_IT(i),i=1,nItPts)

write(*,*)
write(*,*)'============================ Isochore Calculations =========================='
write(*,*)
...
write(*,'(a,11f7.4)')' 1000/T  =',(ThousandOverT_IC(j,1),j=iFirstIcPt,nItPts)
write(*,'(a,11f7.2)')' aDep_IC =',(aDep_IC(j,1),j=iFirstIcPt,nItPts)
...
do i=iFirstIcPt,nItPts
	...
	write(*,'(2A3,A8,2A12,50A9)')"j","i","Tsat","Psat","rhoV","B2sat",&
			"aDepSat","uDep","Hvap","zLiq","zVap","drhoV","d2rhoV"	
	do j=1,maxIter
		if(j==1)then
			zLiq(i)=0.001
		else
			denom=rho_IC(i)*Rgas*Tsat(i)
			if(denom < zeroTol)print*,'ITICmain: denom=0. rho,Tsat=',rho_IC(i),Tsat(i)
			zLiq(i)=Psat(i)*MW/( denom )
		endif
		...
		if(calculateVirialSupercritical .AND. calculateVirialSubcritical) then
			...
			if(ABS(denom) < zeroTol)print*,'ITICmain: denom ~ 0 computing dB2_dBeta(i). denom=',denom  
			dB2_dBeta(i) = (B2sat(i) - B2atHighestT)/denom
		else
			B2sat(i) = B2calc(Tsat(i),TdB2_dTsat)/Mw !JRE20210620
			dB2_dBeta(i) = -Tsat(i)*TdB2_dTsat/Mw    !JRE20210620 Weird? Yes, but beta*dB2/dBeta = -TdB2_dT so mult by Tsat, then divide by Tsat in hSatVap Eq below, minimizing changes in SMR code. 
		endif

		expArgAZ1=aDepSat(i)+zLiq(i)-1.d0
		if(expArgAZ1 > 33)iExpoErr=1
		if(j==1)then
			expArgB2= -2.0d0*B2sat(j) * rho_IC(j)*exp(expArgAZ1) 
			expArgB3= -1.5d0*B3sat(j)*( rho_IC(j)*exp(expArgAZ1) )**2
		else
			expArgB2= -2.0d0*B2sat(i)*rhoV(i)
			expArgB3= -1.5d0*B3sat(i)*rhoV(i)*rhoV(i)
		endif
		...
		zVsat = 1.d0+B2sat(i)*rhoV(i)+B3sat(i)*rhoV(i)*rhoV(i)
		Psat(i) = ( zVsat )*rhoV(i)/MW*Rgas*Tsat(i)
		hSatVap = dB2_dBeta(i)*rhoV(i)/Tsat(i)+zVsat-1	
		hSatLiq = uDepTSat(i)/Tsat(i)+zLiq(i)-1
		Hvap(i) = ( hSatVap - hSatLiq)*Rgas*Tsat(i)/1000.0

		write(*,'(2I3,F8.2,2F12.9,4F9.3,50F9.6)')j,i,Tsat(i),Psat(i),rhoV(i),B2sat(i),&
			aDepSat(i),uDepTsat(i)/Tsat(i),Hvap(i),zLiq(i),zVsat,rhoVp(i),rhoVpp(i) &
			,aDepSat(i) - aDep_IC(i,1)

		!<====Convergence Check==========================================
		PsatIncrement=abs((tempPsat-Psat(i))/Psat(i)*100.0)
		rhoVIncrement=abs((tempRhoV-rhoV(i))/rhoV(i)*100.0)
		if(PsatIncrement < 1e-3 .and. rhoVIncrement < 1e-3) then
			if(rhoVp(i) < 1.d0 .AND. rhoVpp(i) > 0.d0) then
				convergeStatus(i)=1
				Write(*,'(A,I2,A,F8.4,A)') "Isochore",i,":",rho_IC(i)," (g/ml):"
				write(*,'(A19,I3,A)') "Convergence Status= ",convergeStatus(i)," (Converged to first root)"				
				exit
			elseif(rhoVp(i) > 1.d0 .AND. rhoVpp(i) > 0.d0)then
				convergeStatus(i)=2
				Write(*,'(A,I2,A,F8.4,A)') "Isochore",i,":",rho_IC(i)," (g/ml):"
				write(*,'(A19,I3,A)') "Convergence Status= ",convergeStatus(i)," (Converged to second root)"				
				exit
			elseif(rhoVpp(i) < 0.d0) then
				convergeStatus(i)=3
				Write(*,'(A,I2,A,F8.4,A)') "Isochore",i,":",rho_IC(i)," (g/ml):"
		...
		if(Psat(i) < 0.0d0 .OR. rhoV(i) < 0.0d0 .OR. Psat(i) > 10.d0) then 
			convergeStatus(i)=0
			Write(*,'(A,I2,A,F8.4,A)') "Isochore",i,":",rho_IC(i)," (g/ml):"
...
write(*,'(7a)')'                  (K)','      %','              (MPa)','    (g/ml)','      (g/ml)','  (KJ/mol)','    (ml/g)'
write(*,'(A5,A7,A10,A7,A18,A10,A12,11A10)')&
